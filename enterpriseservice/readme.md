...
2. **Enterprise service**, port **8081** (in another terminal):
...
Service listens on **8081**. H2 in-memory schema comes from [`src/main/resources/schema.sql`](src/main/resources/schema.sql), then Hibernate **validates** mappings.
...
```bash
mvn spring-boot:run -Dspring-boot.run.arguments=--mock.services.base-url=http://localhost:8080
```

## API smoke test (`curl`)

**Base URL:** `http://localhost:8081/api/v1` (restart the app after changing `application.properties`).

**Plants** (repeat for `/suppliers` and `/shipments` with `supplierName` / shipment fields):

```bash
curl -s http://localhost:8081/api/v1/plants
curl -s http://localhost:8081/api/v1/plants/1
curl -s -X POST http://localhost:8081/api/v1/plants -H "Content-Type: application/json" \
  -d '{"plantName":"Alpha","location":"Dubai"}'
curl -s -X PUT http://localhost:8081/api/v1/plants/1 -H "Content-Type: application/json" \
...
```bash
curl -s "http://localhost:8081/api/v1/orchestration/snapshot?latitude=25.26&longitude=55.30&vesselRadiusKm=50"
```

## H2 console

With the app running: [http://localhost:8081/h2-console](http://localhost:8081/h2-console) — JDBC URL `jdbc:h2:mem:enterprise`, user `sa`, empty password (defaults from `application.properties`).
